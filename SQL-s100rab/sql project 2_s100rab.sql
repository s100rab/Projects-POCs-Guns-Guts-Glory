...
-- 8th
create or replace view College_A_SJ_V as select
...
-- 16th
-- Combine results for College A and College B
SELECT
    Career_choice,
    ROUND((SUM(CASE WHEN College = 'A' THEN 1 ELSE 0 END) / COUNT(*)) * 100, 4) AS College_A_Percentage,
    ROUND((SUM(CASE WHEN College = 'B' THEN 1 ELSE 0 END) / COUNT(*)) * 100, 4) AS College_B_Percentage
FROM (
    SELECT 'Higher Studies' AS Career_choice, 'A' AS College FROM college_a_hs WHERE PresentStatus = 'Higher Studies'
    UNION ALL
    SELECT 'Self Employed' AS Career_choice, 'A' AS College FROM college_a_se WHERE PresentStatus = 'Self Employed'
    UNION ALL
    SELECT 'Service/Job' AS Career_choice, 'A' AS College FROM college_a_sj WHERE PresentStatus = 'Service/Job'
    UNION ALL
    SELECT 'Higher Studies' AS Career_choice, 'B' AS College FROM college_b_hs WHERE PresentStatus = 'Higher Studies'
    UNION ALL
    SELECT 'Self Employed' AS Career_choice, 'B' AS College FROM college_b_se WHERE PresentStatus = 'Self Employed'
    UNION ALL
    SELECT 'Service/Job' AS Career_choice, 'B' AS College FROM college_b_sj WHERE PresentStatus = 'Service/Job'
) AS combined
GROUP BY Career_choice;
